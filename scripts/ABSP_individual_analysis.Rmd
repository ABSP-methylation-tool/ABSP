...
params:
  ...
  seqname:
    ...
    input: text
  collection:
    label: "Enter collection name (distinct groups, not to compare)"
    value: collection
    input: text
  group:
    label: "Enter condition / group name (to compare) (examples: control, treated...)"
    value: group1
    input: text
  replicate:
    label: "Enter replicate number"
    value: 1
    input: numeric
  clone:
    label: "Enter clone number (for direct sequencing, please leave empty or enter 0)"
    value: 0
    input: numeric
  DNA_seq:
...
```{r TEST SCRIPT files and params, eval=F}
## TEST SCRIPT WITHOUT KNITTING
...
seq_group = "0"
seq_rep = "1"  ; seq_rep = ifelse(seq_rep==0,"",seq_rep)
seq_clone = "0" ; seq_clone = ifelse(seq_clone=="0"|seq_clone==""|seq_clone==" ","",paste0("clone",str_pad(seq_clone, 2, "0", side="left")))
clone_nb = 0
seq_date_s1 = "2022-03-04"
seq_date_s2 = "2022-03-04"
selected_genome = "BSgenome.Hsapiens.UCSC.hg19"

# Get files paths using choose.files function (Windows only)
DNA_seq_path = choose.files(default = "", caption = "Select .fasta file corresponding to DNA sequence",
                                multi = FALSE, filters=Filters, index=nrow(Filters))
sangerseq_s1_path = choose.files(default = "", caption = "Select .ab1 file corresponding to the result of sequencing in one direction #1",
                                multi = FALSE, filters=Filters, index=nrow(Filters))
sangerseq_s2_path = choose.files(default = "", caption = "Select .ab1 file corresponding to the result of sequencing in the other direction #2",
                                multi = FALSE, filters=Filters, index=nrow(Filters))
...
# Create data frame
paramTable <- data.frame("Parameters" = parameters, "Input values" = values)
...
BisConRate_all[,c(2,3)] <- format(round(x = BisConRate_all[,c(2,3)],2), nsmall = 2)
...
BisConRate_all[,2] <- format(round(x = BisConRate_all[,2],2), nsmall = 2)
BisConRate_all_FT <- formattable(BisConRate_all,
                                 align = rep("c",ncol(BisConRate_all)),
                                 width = "50%",
                                 list("Bisulfite conversion rate Forward" = color_condi(">=", 
...
if (useSeqF==T & useSeqR==T & correct_F==T & correct_R==T) {
  ...
  # Order by CG_nb, mean and sd of methylation 
  CG_meth = CG_meth %>% 
    .[with(., order(CG_nb)),] %>% 
    rowwise() %>% 
...
if (!file.exists(legend_bases_path)) { 
  png(filename=legend_bases_path, width=260, height=60)
  par(mai=c(0,0,0,0))
  plot.new()
  legend(x="center", 
         legend=c("A","T","G","C"), 
         fill=c("#43CD80","#D7191C","#FDAE61","#2C7BB6"),
         border="white", 
         bty = "n", 
         cex=1.5,
         horiz=TRUE)
...
legend_grad_path <-  file.path(indivDir$plot,"legend_methylation_gradient.png")

if (!file.exists(legend_grad_path)) { 
  png(filename=legend_grad_path, width=600, height=60)
  par(mai=c(0,0,0,0))
  plot.new()
