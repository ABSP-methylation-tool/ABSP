...
if(useSeqs1==F){TrimOKs1 <- F}
if(useSeqs2==F){TrimOKs2 <- F}
...
```{r Trim text no sequencing}
if (useSeqs1==F) {
  p("No sequencing file provided for sequencing result #1.",
  style="color:#bf3232 ; font-weight: bold ;")
}
if (useSeqs2==F) {
  p("No sequencing file provided for sequencing result #2.",
  style="color:#bf3232 ; font-weight: bold ;")
}
...
alg_posF <- list(
  ...
  subject_F_end = pa_F@subject@range@start+pa_F@subject@range@width-1,
  # F Pattern : Sequencing results forward
  pattern_F_start = pa_F@pattern@range@start,
  pattern_F_end = pa_F@pattern@range@start+pa_F@pattern@range@width-1
)
```
...
alg_posR <- list(
  ...
  subject_R_end = pa_R@subject@range@start+pa_R@subject@range@width-1, 
  # R Pattern : Sequencing results reverse
  pattern_R_start = pa_R@pattern@range@start,
  pattern_R_end = pa_R@pattern@range@start+pa_R@pattern@range@width-1 
)
```
...
```{r text no sF, eval=!useSeqF}
p("No sequencing file provided for forward sequencing result.",
  style="color:#bf3232 ; font-weight: bold ;")
```
...
```{r text no sR, eval=!useSeqR}
p("No sequencing file provided for reverse sequencing result.",
  style="color:#bf3232 ; font-weight: bold ;")
```
...
BisConRate_all <- BisConRate_all[order(BisConRate_all[,1]),]
rownames(BisConRate_all) <- NULL
```
...
if (pa_F@pattern@indel@partitioning@end!=0) {
  QC_indel_seq_F <- data.frame("indel_seq"=indel$sequencing_F)
  colnames(QC_indel_seq_F) <- c("Indel positions in trimmed sequencing result")
  formattable(QC_indel_seq_F, align=c("c","c"))
}
if (pa_F@subject@indel@partitioning@end!=0) {
  QC_indel_ref_F <- data.frame("indel_ref"=indel$dna_ref_F)
  colnames(QC_indel_ref_F) <- c("Indel positions in reference DNA sequence")
  formattable(QC_indel_ref_F, align=c("c","c"))
}
if (pa_F@pattern@indel@partitioning@end==0 & pa_F@subject@indel@partitioning@end==0) {cat("No insertion/deletion in forward alignment.")}
```
...
if (pa_R@pattern@indel@partitioning@end!=0) {
  QC_indel_seq_R <- data.frame("indel_seq"=indel$sequencing_R)
  colnames(QC_indel_seq_R) <- c("Indel positions in trimmed sequencing result")
  formattable(QC_indel_seq_R, align=c("c","c"))
}
if (pa_R@subject@indel@partitioning@end!=0) {
  QC_indel_ref_R <- data.frame("indel_ref"=indel$dna_ref_R)
  colnames(QC_indel_ref_R) <- c("Indel positions in reference DNA sequence")
  formattable(QC_indel_ref_R, align=c("c","c"))
}
if (pa_R@pattern@indel@partitioning@end==0 & pa_R@subject@indel@partitioning@end==0) {cat("No insertion/deletion in reverse alignment.")}
```
...
if (useSeqF==F) { p("No sequencing file provided for forward sequencing result.", style="color:#bf3232 ; font-weight: bold ;")}


# Forward Sequencing provided :
if (useSeqF==T) {
...
if (useSeqR==F) { p("No sequencing file provided for reverse sequencing result.", style="color:#bf3232 ; font-weight: bold ;")}


# Reverse sequencing provided :
if (useSeqR==T) {
